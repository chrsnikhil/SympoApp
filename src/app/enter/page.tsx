...
export default function EnterPage() {
  ...
  return (
    <div className="relative min-h-screen bg-background font-body-md text-on-surface flex flex-col justify-between overflow-hidden">
      {/* 3D Web Shooter interactive background */}
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-gutter py-10 flex flex-col items-center text-center">
        ...
        <form onSubmit={onSubmit} className="relative w-full max-w-md flex flex-col items-center">
          ...
          <div className="relative mb-8 w-full">
            <div className="bg-surface px-8 py-4 comic-border rounded-[2rem] relative z-20 comic-tilt-left">
              <h2 className="font-headline-lg text-headline-lg-mobile text-on-surface uppercase italic leading-none">
                {needsName ? "Name Your Team" : "Enter Your Token or Code"}
              </h2>
            </div>
          ...
          {!needsName ? (
            ...
            <div className="w-full max-w-sm mb-4">
              <label htmlFor="teamName" className="block font-label-sm text-on-surface-variant uppercase text-left mb-2 text-[12px]">
                Team Name
              </label>
              <input
                id="teamName"
                ...
                maxLength={40}
                autoFocus
                className="w-full bg-surface-container-lowest comic-border px-6 py-4 font-headline-lg text-headline-lg-mobile text-center tracking-widest outline-none focus:bg-tertiary-fixed/20 transition-colors"
              />
              <p className="mt-2 font-label-sm text-on-surface-variant text-[11px] uppercase">
                This coin is new — name your team and it&apos;s yours!
              </p>
            </div>
          )}

          {/* Character preview badge */}
          {!looksLikeCode && preview && !needsName && (
          ...
          <div className="mt-8 group">
            ...
            >
              <span className="font-display-xl text-headline-lg-mobile text-on-primary uppercase tracking-widest">
                {busy ? "Checking…" : needsName ? "Claim Hero" : "Enter the Spider-Verse"}
              </span>
            </button>
          </div>
        </form>
