...
async function main() {
  ...
  check("every team independently advances to the connections phase", r1AfterImage.every((r) => r.body?.phase === "connections"), r1AfterImage.map((r) => r.body?.phase));
  ...
  const [solverGuesses, stragglerGuesses] = await Promise.all([
    Promise.all(solvers.map((t) => t.client.post("/api/submit", { event: "quiz", challengeSlug: "connections-1", payload: CONNECTIONS_ANSWER }))),
  ...
  check(
    "solvers who all guessed on the same (first) revealed tile score IDENTICAL reveal-falloff points, not a race-dependent value",
  ...
  check(
    "solvers moved on to memory, stragglers stayed on connections — no cross-contamination",
    solvers.every((_, i) => r1AfterGuesses[i].body?.phase === "memory") && stragglers.every((_, i) => r1AfterGuesses[half + i].body?.phase === "connections"),
    r1AfterGuesses.map((r) => r.body?.phase)
  );
